...
public class BasicAITest {
	...
	/*
	 * The setAgentGoals() for the above test:
	 * // TODO: Testkod nedan
		ArrayList<ICondition> trueGoals = new ArrayList<ICondition>();
		ArrayList<ICondition> falseGoals = new ArrayList<ICondition>();
		ICondition getSword = new BelongsToCondition(props.get(0), agents.get(0).getSelf());
		ICondition gotoParken = new IsAtLocationCondition(agents.get(0).getSelf(), scenes.get(2));
		trueGoals.add(getSword);
		trueGoals.add(gotoParken);
	...
	private static void twoActorsTest() {
		...
		// We find the mushroom and the fork in the kitchen, and the poo in the bathroom
	...
	/* Nedanstående kod genererar målen till testet ovan
	 	// TODO: Testkod nedan
		// Add Mario's goal
		ArrayList<ICondition> marioTrueGoals = new ArrayList<ICondition>();
		ArrayList<ICondition> marioFalseGoals = new ArrayList<ICondition>();
		ICondition getPoo = new BelongsToCondition(props.get(2), agents.get(1)
				.getSelf());
		marioTrueGoals.add(getPoo);
		agents.get(1).setTrueGoals(marioTrueGoals);
		agents.get(1).setFalseGoals(marioFalseGoals);

		// Add Bowser's goal
		ArrayList<ICondition> bowserTrueGoals = new ArrayList<ICondition>();
		ArrayList<ICondition> bowserFalseGoals = new ArrayList<ICondition>();
		ICondition getMushroom = new BelongsToCondition(props.get(1), agents
				.get(0).getSelf());
